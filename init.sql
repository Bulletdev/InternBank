...
CREATE TABLE Transacoes_Conta (
    ...
    FOREIGN KEY (Transacao_Id) REFERENCES Transacoes(Transacoes_Id),
    FOREIGN KEY (Contas_Id) REFERENCES Conta(Conta_Id)
);

-- Inserção de usuários
INSERT INTO Usuario (Nome, CPF, Email, Telefone, Endereco, CEP, Bairro, Cidade, Numero, Senha, enabled)
VALUES
    ('Joao Silva', '12345678901', 'joao@email.com', '11999999999', 'Rua A, 123', '01001000', 'Centro', 'Sao Paulo', '123', 'senha123', 1),
    ('Maria Oliveira', '23456789012', 'maria@email.com', '11988888888', 'Rua B, 456', '02002000', 'Bela Vista', 'Sao Paulo', '456', 'senha234', 1),
    ('Pedro Santos', '34567890123', 'pedro@email.com', '11977777777', 'Rua C, 789', '03003000', 'Liberdade', 'Sao Paulo', '789', 'senha345', 1),
    ('Lorena', '45678901234', 'lorena@email.com', '11966666666', 'Rua D, 101', '04004000', 'Vila Mariana', 'Sao Paulo', '101', 'senha456', 1),
    ('Cinthia Souza', '56789012345', 'cinthia@email.com', '11955555555', 'Rua E, 202', '05005000', 'Pinheiros', 'Sao Paulo', '202', 'senha567', 1),
    ('Amanda Pereirra', '67890123456', 'amanda@email.com', '11944444444', 'Rua F, 303', '06006000', 'Moema', 'Sao Paulo', '303', 'senha678', 1);
...
-- Transações relacionadas a contas
SELECT
    t.Transacoes_id,
    t.Pix_id,
    t.Pagamento_id,
...
-- Transações relacionadas a extrato
SELECT
    t.Transacoes_Id,
    e.Extrato_Id
FROM Transacoes t
         INNER JOIN Transacoes_Conta tc ON tc.Transacao_Id = t.Transacoes_Id
         INNER JOIN Extrato e ON e.Transacoes_id = t.Transacoes_Id;

-- Transações relacionadas a depósito
SELECT
    t.Transacoes_Id,
    d.Deposito_id,
...
-- Transações relacionadas a transferência
SELECT
    t.Transacoes_Id,
    tf.Transferencia_id,
...
FROM Transacoes t
         INNER JOIN Transacoes_Conta tc ON tc.Transacao_Id = t.Transacoes_Id
         INNER JOIN Transferencia tf ON tf.Transferencia_id = t.Transferencia_id;

-- Transações relacionadas a saque
SELECT
    t.Transacoes_Id,
    s.Saque_id,
...
FROM Transacoes t
         INNER JOIN Transacoes_Conta tc ON tc.Transacao_Id = t.Transacoes_Id
         INNER JOIN Saque s ON s.Saque_id = t.Saque_id;
